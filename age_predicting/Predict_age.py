...
USING SFCN-STYLE ARCHITECTURE (5 BLOCKS, GLOBAL AVERAGE POOLING, SINGLE SCALAR OUTPUT)."""
...
class BrainAgeCNN(nn.Module):
    ...
    divisible by 32 (see TARGET_SHAPE). The flattened feature map feeds
    directly into the final linear layer, so the final output is a single scalar (predicted age).
    ...
    def __init__(self, in_channels: int = 1):
        ...
        # 5 pools halve each spatial dim 5x: (128,128,96) -> (4,4,3), times 256 channels.
        ...
        self.regressor = nn.Linear(block_channels[-1] * 4 * 4 * 3, 1)
